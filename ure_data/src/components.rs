...
use crate::{container::Container, group::Group, method::{FromGroup, Method}, util::all_the_tuples};
...
pub trait Component: Sized {
	...
	const DELETE: Method<Range<usize>>;
}
...
impl $crate::components::Component for $name {
	...
	const DELETE: $crate::method::Method<std::ops::Range<usize>> = $crate::method::Method::new($crate::components::default_delete::<Self> as fn(_, _));
}
	};
}
...
where
	<<C as Component>::Container as Container>::Item: Default,
{
	for i in 0..num {
		container.push(Default::default())
	}
}

pub fn default_delete<C: Component>(ContMut(mut container): ContMut<C>, range: Range<usize>) {
	container.delete(range);
}
